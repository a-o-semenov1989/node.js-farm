...
const server = http.createServer((req, res) => {
  //принимает колбэк, в которыи передаем переменные request и response. В случае получения запроса - отправим ответ
  ...
  if (pathname === '/' || pathname === '/overview') {
    ...
    res.writeHead(404, {
      //обязательно указываем код ошибки и хедер до отправки ответа //первыи аргумент - код ответа, второи - его тип - браузер будет ожидать html
...
server.listen(8000, '127.0.0.1', () => {
  //listen принимает параметры: порт и хост и опциональныи аргумент - колбэк, которыи запуститься когда сервер начнет слушать
